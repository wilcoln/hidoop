...
import utils.Utils;
public class FragmenterKV extends Fragmenter{
	...
	public String[] fragmenterFichier(String emplacementDuFichier, int tailleMax, String emplacementDesFrags) throws IOException {
        ...
        // un eventuel Key ou Value coincé à la fin de la ligne
        String motRestant = "";

        // la ligne à ajouter dans fragment après avoir corrigé des eventuels KV (au début ou en fin de la ligne)
        ...
        while ((ligne = bufReader.readLine()) != null) {
            ...
            ligneAAjouter = ligne;
            //  Ici on verifie qu'on n'a pas de valeur sans clé c-a-d, pas de "clé<->\nvaleur" ou de "clé\n<->valeur"
            // Et si on est dans ce cas il faut garder la clé pour l'integer dans la ligne suivante
            if (!Fragmenter.IsKeyValue(Fragmenter.LastWord(ligne))){
                ligneAAjouter = Fragmenter.LineWithoutLastWord(ligne);
            }
            ...
            // il faut garder la valeur perdu pour l'inserer dans la ligne suivante
            if (!Fragmenter.IsKeyValue(Fragmenter.LastWord(ligne))) {
                motRestant = Fragmenter.LastWord(ligne);
            } else {
                motRestant = "";
            }
